...
public class OrderCalculationService {
  ...
  public CalculatedAmounts calculate(
      BigDecimal gross,
    ...
    BigDecimal commissionAmount =
        gross.multiply(commissionRatePercent).divide(hundred, SCALE, ROUND);

    BigDecimal gstOnCommission =
        commissionAmount.multiply(gstRatePercent).divide(hundred, SCALE, ROUND);

    BigDecimal netExpected =
        gross.subtract(commissionAmount).subtract(gstOnCommission).setScale(SCALE, ROUND);

    BigDecimal mismatchAmount = netReceived.subtract(netExpected).setScale(SCALE, ROUND);
